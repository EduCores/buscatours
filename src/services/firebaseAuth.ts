...
import { getAuth, Auth, signInWithEmailAndPassword, onAuthStateChanged, User, GoogleAuthProvider, OAuthProvider, signInWithPopup } from 'firebase/auth';
...
export async function ensureEmulatorUser(): Promise<User | null> {
  ...
  if (import.meta.env.VITE_USE_EMULATORS === 'true') {
    try {
      ...
      // User doesn't exist in Auth emulator yet - this is expected for initial setup
      // The frontend will use FALLBACK_USERS from useAppData.js
      console.warn('Emulator auto-login failed (expected on first run). Using fallback user data.');
    }
  }
  return null;
}
